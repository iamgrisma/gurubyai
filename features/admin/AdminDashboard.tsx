...
import { 
    Users, LayoutDashboard, DollarSign, ScrollText,
    UserPlus, Menu, ChevronsLeft, ChevronsRight, Layers, LogOut, Shield, CreditCard
} from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'concierge' | 'users' | 'services' | 'gotras' | 'financials' | 'verification' | 'topups'>('overview');
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex font-sans">
        ...
        {/* Sidebar - Dark Mode for Professional Look */}
        <aside className={`
            fixed top-0 left-0 z-50 h-full bg-stone-900 flex flex-col
        ...
        `}>
            ...
            <div className="p-4 border-t border-stone-800 bg-stone-900">
                 ...
                 <button onClick={() => signOut()} className={`w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-red-400 hover:bg-red-900/20 hover:text-red-300 rounded-lg transition-colors ${isSidebarCollapsed ? 'lg:px-0' : ''}`}>
