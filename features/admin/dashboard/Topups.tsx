  ...
  const processTopup = useMutation({
      mutationFn: async ({ id, status, userId, amount }: { id: string, status: 'approved' | 'rejected', userId: string, amount: number }) => {
          ...
          if (status === 'approved') {
              ...
              // Using the secure RPC admin_add_credits would be cleaner, but logic here is specific to request processing
              // Let's call the RPC manually or stick to the previous logic if RPC doesn't support request linkage easily.
              // For safety, we should use the RPC `admin_add_credits` which handles transaction logging too.
              // However, `admin_add_credits` creates a "Admin Top-up" transaction description.
              // Ideally we want "Top-up Request Approved".
              // For now, let's stick to the direct logic but ensure RLS allows it?
              // WAIT: We revoked direct UPDATE/INSERT on transactions/profiles for non-service-role.
              // Admin needs RLS policy or RPC. `profiles` allows Admin update. `transactions` has revoked INSERT.
              // We MUST use the RPC `admin_add_credits` or create a new RPC `process_topup_request`.
              
              // Let's use `admin_add_credits` for simplicity and consistency with the new security model.
              // The transaction description will be generic but it works.
              ...
              // 4. Notify User
              // Notifications table allows Admin insert via policy? 
              // "Admins manage notifications" policy allows ALL. So insert is fine for Admin user.
      ...
      onSuccess: () => {
          ...
          queryClient.invalidateQueries({ queryKey: ['adminUsers'] }); // Balance updates
